...
// generic compute kernel
template <typename VectorT, typename ScalarT>
void gemm_plain_kernel(int TA, int TB, int M, int N, int K,
    ScalarT ALPHA, 
    VectorT *A, int lda,
    VectorT *B, int ldb,
    ScalarT BETA,
...
/**
 * It's a C++ template issue that I can only implement specific compute function here
 * Will it be better if I implement it in template class?
 */
template <>
void gemm_plain_kernel<float, float>(int TA, int TB, int M, int N, int K, 
    float ALPHA, 
    float *A, int lda,
    float *B, int ldb,
    float BETA,
...
template <>
void gemm_plain<float,float>(int TA, int TB, int M, int N, int K,
    float ALPHA, 
    float *A, int lda,
    float *B, int ldb,
    float BETA,
    float *C, int ldc)
{
  ...
  // main calculation
  gemm_plain_kernel<float,float>(TA,TB,M,N,K,ALPHA,A,lda,B,ldb,BETA,C,ldc);
